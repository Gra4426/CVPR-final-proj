...
def convert(infile, outprefix):
    ...
    ffit.write(
        '% # columns="iteration, evaluation, sigma, axis ratio, bestever, best, median, worst objective function value, further objective values of best, seed=' + seed + ', ' + rdate + '\n')
    ...
    faxlen.write(
        '%  columns="iteration, evaluation, sigma, max axis length,  min axis length, all principle axes lengths  (sorted square roots of eigenvalues of C)", seed=' + seed + ', ' + rdate + '\n')
    fstddev = open(outprefix + 'stddev.dat', 'w')
    fstddev.write(
        '% # columns="iteration, evaluation, sigma, void, void,  stds==sigma*sqrt(diag(C))", seed=' + seed + ', ' + rdate + '\n')

    i = 0
    for fe, si, ka, bs, fv, ms, ws, Me, me in np.nditer(
            [fevals, sigma, kappa, best_seen_fvalue, fvalue, median_seen_fvalue, worst_seen_fvalue, max_eigenv,
             min_eigenv]):
